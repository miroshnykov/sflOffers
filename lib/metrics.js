...
console.log(`PROJECT FOR METRICS:${project}`)
...
exports.sendMetricsSystem = function (fileSizeInfo, clientsCount) {

    const {offer, campaign, affiliates, affiliateWebsites} = fileSizeInfo

    if (config.env === 'development') return
    // console.log('metrics fileSizeOffer:', fileSizeOffer)
    // console.log('metrics fileSizeCampaign:', fileSizeCampaign)
    ...
    let fields = {
        ...
        file_size_recipe_campaigns: campaign,
        file_size_recipe_offers: offer,
        file_size_recipe_affiliates: affiliates,
        file_size_recipe_affiliatewebistes: affiliateWebsites,
    ...
    cpu.usage(function (cpu) {
        ...
        if (load_cpu && load_cpu > 0 && num_cpu) {

            fields.cpu_avg_perc = load_cpu / num_cpu
        } else {
            fields.cpu_avg_perc = 0
        }
...
exports.sendSizeOfSegments = function (memorySizeOfSegment) {

    if (config.env === 'development') return
    let fields = {
        size_segments: memorySizeOfSegment,
    }
    cpu.usage(function (cpu) {
        clientInfluxdb.write(project + '_system')
            .tag({
...
exports.sendMetricsCountOfClients = (countOfClients) => {
    if (config.env === 'development') return
